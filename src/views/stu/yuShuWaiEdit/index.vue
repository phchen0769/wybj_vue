        ...
        <Form ref="searchForm" :model="searchForm" inline :label-width="0">
          ...
          >
            <Form-item style="margin-left: 10px" class="br">
              <Button @click="handleSearch" type="primary" icon="ios-search" size="small" ghost
                >搜索</Button
              >
              <Button @click="handleReset" type="warning" size="small" icon="md-refresh" ghost
                >重置</Button
              >
              <Button @click="excelData" type="success" icon="md-paper-plane" size="small" ghost
                >导出</Button
              >
            </Form-item>
...
<script>
import { getStudentEvaluateList } from './api.js'
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              'span',
              params.index + (this.searchForm.pageNumber - 1) * this.searchForm.pageSize + 1
            )
          }
        },
        {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true
      getStudentEvaluateList().then(res => {
        this.loading = false
        if (res.success) {
